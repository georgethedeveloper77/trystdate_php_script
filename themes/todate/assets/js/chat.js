...
function send_chat_message(msg){
    $('.chat_message_any_media_progress').removeClass('hide');
	$('#btn_chat_f_send').prop("disabled", true);
    let lastmsg = $('.messages:last').attr('data-lastid');
    ...
    $.ajax({
        ...
        success: function (data) {
			...
			$('#btn_chat_f_send').prop("disabled", false);
            $('.chat_message_any_media_progress').addClass('hide');
...
(function($){
    $(function(){
		...
		$('.chat_stts_dropd, .chat_useropts_dropd').dropdown({
			alignment: 'right'
		});
		$( document ).on( 'change', '#chat_go_online input', function(e){
